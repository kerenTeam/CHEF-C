/**
 * jQuery KeRenUI 4.1
 *成都可人软件有限公司 Copyright © Keren 2015
 */
//样式
$(function () {
    readyIndex();
...
function delConfirm(url, parm, msg) {
    confirmDialog("可人提示", msg, function (r) {
        if (r) {
            Loading(true, "正在删除数据...");
...
function delConfig(url, parm, count) {
    ...
    confirmDialog("可人提示", "注：您确定要删除 " + count + " 笔记录？", function (r) {
        if (r) {
            Loading(true, "正在删除数据...");
            window.setTimeout(function () {
                AjaxJson(url, parm, function (data) {
...
function AddTabMenu(tabid, url, name, img, Isclose, IsReplace, IsVisitorModule) {
    ...
    if (top.document.getElementById("tabs_" + tabid) == null) { //如果当前tabid存在直接显示已经打开的tab
        ...
        if (tabs_container.find('li').length >= 7) {
            Loading(false);
            alertDialog("为保证系统效率,只允许同时运行7个功能窗口,请关闭一些窗口后重试！", 0)
            return false;
        }
